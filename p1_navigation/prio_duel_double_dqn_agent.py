...
class PrioDuelDDQNAgent():
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if self.t_step == 0:
            ...
            if len(self.memory) > BATCH_SIZE:
                beta = min(1.0, BETA_START + self.step_cnt*(1.0 - BETA_START)/BETA_MAX_STEP)
                experiences = self.memory.sample(beta)
                priorities = self.learn(experiences, GAMMA)
                indices = experiences[0]
    ...
    def learn(self, experiences, gamma):
        ...
        """
        indices, states, actions, rewards, next_states, dones, weights = experiences
        ...
        losses = weights*(Q_expected - Q_targets)**2
        loss  = losses.mean()
        # Minimize the loss
        ...
        priorities = losses.detach().cpu().squeeze(1).numpy()
...
#            self.priorities.append(max_prio)
    ...
    def sample(self, beta):
        ...
        ws = torch.from_numpy(np.vstack(weights)).float().to(device)
  
        return (indices, states, actions, rewards, next_states, dones, ws)
